function varargout = ISEEU(varargin)
% ISEEU MATLAB code for ISEEU.fig
%      ISEEU, by itself, creates a new ISEEU or raises the existing
%      singleton*.
%
%      H = ISEEU returns the handle to a new ISEEU or the handle to
%      the existing singleton*.
%
%      ISEEU('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ISEEU.M with the given input arguments.
%
%      ISEEU('Property','Value',...) creates a new ISEEU or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ISEEU_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ISEEU_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ISEEU

% Last Modified by zefeng v1.0 14-08-2019 07:
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @ISEEU_OpeningFcn, ...
                   'gui_OutputFcn',  @ISEEU_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ISEEU is made visible.
function ISEEU_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ISEEU (see VARARGIN)
...
filename = [pwd '\couvertureISEEU.jpg'];
PIC = imread(filename);
axes(handles.axes2)
image(PIC);
axis off

%text(1400,700,'ISEEU SYSTEM','fontsize',18,'color','w');
...
% UIWAIT makes ISEEU wait for user response (see UIRESUME)
% uiwait(handles.login_figure);


% --- Outputs from this function are returned to the command line.
function varargout = ISEEU_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes on button press in ISEEU.
function login_Callback(hObject, eventdata, handles)
% hObject    handle to ISEEU (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
